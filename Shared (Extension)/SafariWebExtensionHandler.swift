...
//  Shared (Extension)
//
//  Created by TAEYANG KIM on 3/24/23.
//
...
func prepareRequest(prompt: String, apiKey: String) -> URLRequest {
    ...
    let params = requestBody(model: "text-davinci-003", temperature: 0, max_tokens: 360, prompt: prompt)
...
class SafariWebExtensionHandler: NSObject, NSExtensionRequestHandling {
    ...
    func beginRequest(with context: NSExtensionContext) {
        ...
        messageContent = "Check if this site's content is abusive and dangerous: " + "\"" + messageContent + "\""
        if #available(macOSApplicationExtension 11.0, *) {
